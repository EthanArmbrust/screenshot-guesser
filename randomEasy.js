...
function checkUserInput(){
	...
	if(hintOn){
		...
		if(!isGameOver){
		var radioAnswer = document.querySelector('input[name = "selection"]:checked').value;
			console.log(radioAnswer);
			
		if(radioAnswer == "first"){
			var selectedAns = 1;
		}
		if(radioAnswer == "second"){
			var selectedAns = 2;
		}
		if(radioAnswer == "third"){
			var selectedAns = 3;
		}
		if(radioAnswer == "fourth"){
			var selectedAns = 4;
		}
...
showEpisodeName = document.createTextNode(episodeName);
	...
	correctAnswer = getRandomIntInclusive(1,4);
	
	if(correctAnswer === 1){
		document.getElementById("firstChoice").innerHTML = episodeName;
	}
	if(correctAnswer === 2){
		document.getElementById("secondChoice").innerHTML = episodeName;
	}
	if(correctAnswer === 3){
		document.getElementById("thirdChoice").innerHTML = episodeName;
	}
	if(correctAnswer === 4){
		document.getElementById("fourthChoice").innerHTML = episodeName;
	}
	...
	for (i = 0; i < 3; i++) {
		...
		wrongSeason = getRandomIntInclusive(1,10);
		wrongEpisode = getRandomIntInclusive(1, episodeNumbers[wrongSeason]);
		switch(wrongSeason){
	case 1:
	wrongEpisodes[i] = s1Titles[wrongEpisode];
	break;
 	case 2:
        wrongEpisodes[i] = s2Titles[wrongEpisode];
	break;
	case 3:
        wrongEpisodes[i] = s3Titles[wrongEpisode];
	break;
	case 4:
        wrongEpisodes[i] = s4Titles[wrongEpisode];
	break;
	case 5:
        wrongEpisodes[i] = s5Titles[wrongEpisode];
	break;
	case 6:
        wrongEpisodes[i] = s6Titles[wrongEpisode];
	break;
	case 7:
       wrongEpisodes[i] = s7Titles[wrongEpisode];
	break;
	case 8:
        wrongEpisodes[i] = s8Titles[wrongEpisode];
	break;
	case 9:
        wrongEpisodes[i] = s9Titles[wrongEpisode];
	break;
	case 10:
        wrongEpisodes[i] = s10Titles[wrongEpisode];
	break;
}
	...
	var choices = [1, 2, 3, 4];
	var index = choices.indexOf(correctAnswer);
	
	if(index > -1){
		choices.splice(index, 1);
	}
	
	for(zray = 0; zray < 3; zray++){
	
	if(choices[zray] === 1){
		document.getElementById("firstChoice").innerHTML = wrongEpisodes[zray];
	}
	if(choices[zray] === 2){
		document.getElementById("secondChoice").innerHTML = wrongEpisodes[zray];
	}
	if(choices[zray] === 3){
		document.getElementById("thirdChoice").innerHTML = wrongEpisodes[zray];
	}
	if(choices[zray] === 4){
		document.getElementById("fourthChoice").innerHTML = wrongEpisodes[zray];
	}
	}
